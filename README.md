...
완성된 한글 패치: **[`patch/SRWK-Korean-v1.11.xdelta`](patch/SRWK-Korean-v1.11.xdelta)** (약 1.6 MB, xdelta3 / VCDIFF)

1. xdelta 적용 도구(xdelta UI, Delta Patcher 등) 또는 명령줄:
   ```
   xdelta3 -d -s "Super Robot Wars K (Japan).nds" SRWK-Korean-v1.11.xdelta "Super Robot Wars K (Korean).nds"
   ```
2. **기준 ROM** (본인이 합법적으로 덤프한 것):
   - `Super Robot Wars K (Japan).nds` — **67,108,864 바이트, CRC32 `D16DB8AF`**
3. 결과 ROM:
   - `Super Robot Wars K (Korean).nds` — **63,693,664 바이트, CRC32 `D124A344`**

> **v1.11 변경**: **타이틀 로고 리뉴얼** — 부끄지 님이 새로 그린 「슈퍼로봇대전K」 로고 아트로 교체 (add02 #2359 팔레트 최적화 + #2360 타일, 원본 스타일의 금색 레터링·빨간 판).
...
| 전투 특수능력·상태 라벨판 (4장) | `data/add02dat.bin` #2205·2207·2209·2211 | ✅ 한국어 (카운터·회피·분신·전자실드·조준치·특수효과무효 등, 타일 아틀라스 in-place, v1.7) |
| 전투 커맨드 아이콘 `攻反防避` | `data/add02dat.bin` #1889 | ✅ 「공반방피」 (아이콘 시트 부분 교체, v1.7) |
| `無敵` 컷인 | `data/add04dat.bin` #9566 | ✅ 「무적」 (일러스트 인페인트 후 재작도, v1.7) |

> **전수조사 완료**: 전 이미지 그래픽 7,447개(add02/04/08) 전수 렌더 + 다단계 시각감사로 잔여 미번역 일본어 이미지를 모두 확인·처리 (v1.7).
